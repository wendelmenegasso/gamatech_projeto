...
public class BancoDeDados {
	...
	public void conectar() {
		...
		String servidor = "jdbc:mysql://localhost/b3tech_projeto";
	...
	public void atualizaCliente(String numero_conta_cliente, String nome,String eMail, String telefone) throws SQLException {
		
		//Se o campo nome que foi passado como parâmetro estiver vazio, significa que ele não foi
		...
		//Se o campo E-Mail que foi passado como parâmetro estiver vazio, significa que ele não foi
		//preenchido, portanto não precisa ser atualizado
		if(!eMail.equals("")) {
			String query = "UPDATE clientes SET email_cliente='" +eMail+ "' WHERE numero_conta="+numero_conta_cliente+";";
			System.out.println(query);
			this.statement.executeUpdate(query);
		}
		
		//Se o telefon que foi passado como parâmetro estiver vazio, significa que ele não foi
		//preenchido, portanto não precisa ser atualizado
		if(!telefone.equals("")) {
			String query = "UPDATE clientes SET celular_cliente='" +telefone+ "' WHERE numero_conta="+numero_conta_cliente+";";
			System.out.println(query);
			this.statement.executeUpdate(query);
		}
	}
	
	//Método para Incluir as Contas, caso consiga incluir, retorna true, caso conntrário false.
	public boolean incluirContas(String agencia_contas, String tipo_conta, String saldo_contas, String numero_contas) {
		boolean ret = false;
		try {
			String query = "INSERT INTO contas (numero_conta,numero_agencia,tipo_conta,saldo_conta) VALUES ('"+numero_contas+"','"+
		agencia_contas+"','"+tipo_conta+"',"+saldo_contas+");";
	...
	//Método para Excluir as Contas, caso consiga incluir, retorna true, caso conntrário false.
	...
	public String extrato(String numero_conta) {
		...
		String html = "";
			...
			html = "<!DOCTYPE html>"
					...
					+ ""
					+ "										<input type=text id=agencia value="+resultset.getInt("numero_agencia")+ " align=center disabled>"
					+ "									</div>"
					+ "									<label>Conta</label><br>"
					+ ""
					+ "										<input type=text id=conta value="+resultset.getString("numero_conta")+" align=center disabled>"
					+ "									</div>"
					+ "								</div>"
					+ ""
					+ "";
